  ...
  async _flushPush() {
    ...
    for (const dataType of types) {
      try {
        ...
        const pushed = await this.client.push(this.userHash, dataType, payload, version);
        // Adopt the server-assigned version when available — the server
        // computes it from the stored row (not the client's guess), so this
        // keeps concurrent devices monotonic.
        this.remoteVersions[dataType] =
          pushed && typeof pushed.version === "number" ? pushed.version : version + 1;
      } catch (err) {
