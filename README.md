...
- For dev work: **Node `^20.19.0 || >=22.12.0`, and npm 11+** (`npm install -g npm@11`). The Node floor is not new — `vite` and `rolldown` have required `^20.19.0 || >=22.12.0` for some time, so the previous "Node 18+" here was already wrong. The npm floor is enforced: npm 10 silently strips the `libc` platform fields from `package-lock.json`, so `package.json` declares `devEngines.packageManager` and npm below 11 refuses to install rather than quietly degrading the lockfile.
